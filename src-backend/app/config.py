...
# Always resolve paths relative to THIS file (backend/app/ or src-backend/app/),
# not relative to wherever the process was launched from.
_APP_DIR = os.path.dirname(os.path.abspath(__file__))
_BACKEND_DIR = os.path.dirname(_APP_DIR)  # backend/ or src-backend/
...
class Settings:
    ...
    def __init__(self):
        ...
        # To make it consistent between src-backend and backend, let's resolve to the root project folder's backend/ dir if it exists
        backend_db_dir = os.path.join(_PROJECT_DIR, "backend")
        if os.path.exists(backend_db_dir):
            self.db_path = os.path.join(backend_db_dir, "bengali_ocr.db")
        else:
